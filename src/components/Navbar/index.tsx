...
const Navbar = () => {
	return (
		<div className='sticky top-0 right-0 backdrop-blur-xl backdrop-brightness-200 bg-[rgba(0,2,18,0.85)] z-[999]'>
			<div className='flex flex-row w-full h-14 max-[414px]:h-12 max-w-7xl max-[414px]:max-w-full justify-between align-middle my-0 mx-auto'>
				<div className='flex flex-row items-center gap-8 py-8 max-[414px]:py-0'>
					<Link href='/' className="group flex flex-row items-center">
						<svg xmlns="http://www.w3.org/2000/svg" version="1.0" viewBox="0 0 1600 1200" className='h-11 w-auto -mr-[20px] max-[414px]:h-9 max-[414px]:-mr-[17px] -ml-7 group-hover:text-indigo-500'>
						...
						<p className='text-md max-[414px]:text-[12px] font-sans text-white cursor-pointer'>oundWave</p>
					</Link>
					{/* <Link href='/'>
						<p className='cursor-pointer text-sm font-sans hover:text-zinc-400'>Features</p>
					</Link> */}
					<Link href='/pricing'>
						<p className='cursor-pointer text-md max-[414px]:text-[12px] font-sans hover:text-zinc-400'>Pricing</p>
					</Link>
					{/* <Link href='/'>
				...
				<div className='flex flex-row items-center gap-6'>
					<button className='text-md max-[414px]:text-[12px] font-sans text-white rounded-md p-0 hover:text-zinc-400'>
            Log in
					</button>
					<button
						className='text-md max-[414px]:hidden max-[414px]:py-12 font-sans bg-indigo-700 border border-indigo-700 text-white rounded-3xl px-4 py-0.5 transition ease-in-out delay-75 hover:shadow-[0_0_20px_5px_rgba(99,102,241,0.5)]'
					>
            Sign up
					</button>
				</div>
